    ...
    if st.button("Load Incidents"):        
        ...
        if len(df)>0:
            ...
            fig1 = px.bar(
                ...
                color="Status",
                barmode="group",
                title="Incidents Opened and Closed per Week (Monday to Sunday)"
            )
            ...
            x="Week",
            y="Resolution Rate (%)",
            title="Weekly Incident Resolution Rate",
            markers=True)
    ...
    if st.button("Load Service Requests"):
        ...
        if len(df)>0:        
            ...
            fig1 = px.bar(
                ...
                color="Status",
                barmode="group",
                title="Requests Opened and Closed per Week (Monday to Sunday)"
            )
            ...
            x="Week",
            y="Resolution Rate (%)",
            title="Weekly Requests Resolution Rate",
            markers=True)
            fig2.update_traces(line=dict(color="green", width=3))
            st.plotly_chart(fig2, use_container_width=True)
